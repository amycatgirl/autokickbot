  ...
  for (const element of commandFolders) {
    if (fs.lstatSync(`./commands/${element}`).isDirectory()) {
      const commandFiles = fs
        .readdirSync(`./commands/${element}`)
        .filter((file) => file.endsWith(".js"));
      for (const file of commandFiles) {
        const { default: command } = await import(
          `./commands/${element}/${file}`
        );
...
sub.psubscribe("__keyevent@0__:expired", async () => {});
...
client.once("ready", () => {
  ...
  Log.d(
    "bot",
    `Logged in as ${client.user?.username}. Watching ${
      Array.from(client.servers.values()).length
    } servers.`
  );
});
...
client.on(
  "packet",
  async (p) => p.type === "ServerCreate" && (await guildJoin(p, client))
);
